...
const ConfirmDialog = ({ openDialog, setOpenDialog, name,id_condominio, onDeleteSuccess }: dialogProps) => {
			...
			<AlertDialog.Content className="fixed left-1/2 top-1/2 max-h-[85vh] w-[60vw] max-w-[400px] -translate-x-1/2 -translate-y-1/2 rounded-md bg-white p-[25px] shadow-[var(--shadow-6)] focus:outline-none data-[state=open]:animate-contentShow text-center z-50">
				...
				<div className="flex justify-center gap-[10px]">
					...
					<AlertDialog.Action asChild>
						<button className="inline-flex h-[35px] w-full items-center justify-center rounded bg-red-500 px-[15px] font-medium leading-none text-white outline-none outline-offset-1 hover:bg-red-400 select-none" onClick={() => deleteCondominioFunc()}>
							Excluir
						</button>
					</AlertDialog.Action>
				</div>
